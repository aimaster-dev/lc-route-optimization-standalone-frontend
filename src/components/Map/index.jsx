...
const Map = ({ route_id, showOptimizeRoute, showComparison }) => {
  ...
  const fetchRouteData = async () => {
    try {
      ...
      if (showComparison) {
        ...
        routeData = await getComparisonMap(route_id);
        ...
        if (showOptimizeRoute) {
          routeData = await optimizeRouteData(route_id);
          setoptimzeHtml(routeData.html);
          setManualHtml(null);
        } else {
          routeData = await manualRouteData(route_id);
  ...
  }, [route_id, showOptimizeRoute, showComparison]);
